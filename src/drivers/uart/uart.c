...
static void enable_uart_clock(int port, int enable)
{
	switch (port) {
	case 0:
		clock_switch(CLK_UART0, enable);
		break;
	case 1:
		clock_switch(CLK_UART1, enable);
		break;
	case 2:
		clock_switch(CLK_UART2, enable);
		break;
	case 3:
		clock_switch(CLK_UART3, enable);
		break;
	default:
		break;
	}
...
#define OVERSAMPLE		(16)
	...
	wr_regl(port, UIER, 0);	// disable interrupts
	wr_regl(port, UICR, 0xffffffff);	// clear interrupts

	uart_clk = __get_uart_clock_rate(port);
	ucr_val = UCR_OVERSAMPLE_RATE(OVERSAMPLE);

	wr_regl(port, UCR, UCR_TXFIFO_RESET | UCR_RXFIFO_RESET);	// FIFOs reset

	wr_regl(port, UMR, UMR_VALUE_OVERSAMPLE(uart_clk, OVERSAMPLE, baudrate));	// baudrate set
...
void uart_putc(int port, const char c)
{
	...
	do {
		tx_dlt_depth  = (rd_regl(port, USR) & USR_TFLEVEL_MASK) >> USR_TFLEVEL_SHIFT;
	} while (tx_dlt_depth == TX_FIFO_DEPTH && --timeout);
...
void uart_flush_fifo(int port, int dir)
{
	unsigned long ucr = rd_regl(port, UCR);

	if (dir & UART_TX)
		ucr |= UCR_TXFIFO_RESET;
	if (dir & UART_RX)
		ucr |= UCR_RXFIFO_RESET;

	wr_regl(port, UCR, ucr);
}

void uart_tx_drain(int port)
{
	polling_bits_clear_all(GET_REG, portaddr(port, USR), USR_TFLEVEL_MASK, 100000); 
}
*/
